...
#if defined(NDEBUG)
# error "CrescentCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7fb819d9017922a38fd3b069c1b6169d89a9858c751dd930de79f080ff42f718");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CrescentCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CrescentCoin Signed Message:\n";
...
{
    // CrescentCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 566 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 50046316 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CrescentCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CrescentCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1514534430, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "1514499639";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 566 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044c5b423071d02447351a44c269f1c29842fe33c6435d16f43abaf372f3551640b988d842d9d94ffd089bcb664ae286880d90a5040bc6a8522e333118100281dd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514534430;
        ...
        assert(block.hashMerkleRoot == uint256("0xdc10f05552a2be137fbfab0d8439f34df1d83f61e5c52b3aa9c0025d8eed18fd"));
...
unsigned char pchMessageStart[4] = { 0xc7, 0x79, 0x82, 0x3d }; // CrescentCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrescentCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrescentCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrescentCoinMiner(CWallet *pwallet)
{
    printf("CrescentCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crescentcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CrescentCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CrescentCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrescentCoinMiner, pwallet));
}
